...
import { Download, FileSpreadsheet, Loader2, PieChart, TrendingUp, Calendar, CheckCircle2 } from 'lucide-react';
...
export default function LWFExportPage() {
  const [formData, setFormData] = useState<ExportFormData>({
    unit: '',
    year: '',
    consolidationType: 'single' // 'single', 'unit-year', 'all-units-year', 'custom'
  });
  ...
  const handleExport = async (e: React.FormEvent) => {
    ...
    if (formData.consolidationType === 'single') {
      ...
      if (selectedUnits.length === 0 || selectedYears.length === 0) {
        toast.error('Please select at least one unit and year');
        return;
      }
    }
    ...
    try {
      ...
      const response = await fetch('/api/exports/lwf', {
        method: 'POST',
        headers: {
      ...
      let filename = '';
      if (formData.consolidationType === 'single') {
        filename = `LWF_Report_${formData.unit}_${formData.year}.xlsx`;
      } else if (formData.consolidationType === 'unit-year') {
        filename = `LWF_Reports_${formData.unit}_${formData.year}.xlsx`;
      } else if (formData.consolidationType === 'all-units-year') {
        filename = `LWF_all_units_${formData.year}.xlsx`;
      } else if (formData.consolidationType === 'custom') {
        const timestamp = new Date().getTime();
        filename = `LWF_custom_${timestamp}.xlsx`;
      }
      ...
      toast.success('LWF report generated successfully!');
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to generate LWF report');
    } finally {
      setTimeout(() => {
        setIsLoading(false);
        setProgress(0);
  ...
  const isFormValid = (() => {
    if (formData.consolidationType === 'single') {
      ...
      return selectedUnits.length > 0 && selectedYears.length > 0;
    }
    return false;
  })();
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 lg:px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">LWF Export</h1>
          </div>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    { id: 'single', label: 'Single Report', sub: 'One Unit, One Year' },
                    { id: 'unit-year', label: 'Unit Annual', sub: 'One Unit, One Year' },
                    { id: 'all-units-year', label: 'All Units Annual', sub: 'All Units, One Year' },
